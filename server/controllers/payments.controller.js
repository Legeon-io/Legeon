...
export const verifyIFSC = async (req, res) => {
  ...
  try {
    res
      ...
      .json({ message: "IFSC Code is valid", response: response });
  } catch (error) {
...
export const saveAccountDetails = async (req, res) => {
  try {
    ...
    const decryptedData = {
      accountType: CryptoJS.AES.decrypt(
        encryptedData.accountNumber,
        encryptionKey
      ).toString(CryptoJS.enc.Utf8),
      accountHolderName: CryptoJS.AES.decrypt(
        encryptedData.accountHolderName,
        encryptionKey
      ).toString(CryptoJS.enc.Utf8),
      ifscCode: CryptoJS.AES.decrypt(
    ...
    const response = await axios.post(
      `${process.env.DATABASE_SERVER_URL}/api/masterkeys/decryptMasterKey`,
      { username }
    );
    ...
    const encryptedDataWithMasterKey = {
      username: username,
      accountType: CryptoJS.AES.encrypt(
        decryptedData.accountType,
        masterKey
      ).toString(),
      accountHolderName: CryptoJS.AES.encrypt(
        decryptedData.accountHolderName,
        masterKey
      ).toString(),
      ifscCode: CryptoJS.AES.encrypt(
        decryptedData.ifscCode,
        masterKey
      ).toString(),
      accountNumber: CryptoJS.AES.encrypt(
        decryptedData.accountNumber,
        masterKey
      ).toString(),
    };

    const existingAccountDetails = await BankAccountDetails.findOne({
      username,
    });

    if (existingAccountDetails) {
      existingAccountDetails.accountType =
        encryptedDataWithMasterKey.accountType;
      existingAccountDetails.accountHolderName =
        encryptedDataWithMasterKey.accountHolderName;
      existingAccountDetails.ifscCode = encryptedDataWithMasterKey.ifscCode;
      existingAccountDetails.accountNumber =
        encryptedDataWithMasterKey.accountNumber;
    ...
    res
      .status(200)
      .json({ message: "Data encrypted and stored successfully!" });
  } catch (error) {
